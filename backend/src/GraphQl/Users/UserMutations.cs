...
public sealed class UserMutations
{
    ...
    )
    {
        ...
        if (!identityResult.Succeeded)
        {
            ...
            {
                // Ignore `*UserName` errors that have corresponding `*Email` errors because we use `Email` as `UserName`.
                if (error.Code is not "DuplicateUserName"
                    and not "InvalidUserName")
                {
                    errors.Add(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await CommonAuthorization.IsAtLeastAssistantOfVerifiedInstitution(user, input.InstitutionId, context, cancellationToken).ConfigureAwait(false))
        {
            return new AllowRepresentativeToSignDataPayload(
                new AllowRepresentativeToSignDataError(
                    AllowRepresentativeToSignDataErrorCode.UNAUTHORIZED,
                    $"User can not be granted signing permission.",
                    []
                )
            );
        }
        ...
        if (!identityResult.Succeeded)
        {
            ...
            {
                errors.Remove(
                    // TODO Which error codes occur here? When known, translate the properly into `AllowUserToSignDataErrorCode`.
                    error.Code switch
                    {
                        _ =>
                            new AllowRepresentativeToSignDataError(
                                AllowRepresentativeToSignDataErrorCode.UNKNOWN,
                                $"{error.Description} (error code `{error.Code}`)",
                                [nameof(input)]
                            )
                    }
            ...
            return new AllowRepresentativeToSignDataPayload(user, errors);
        }

        return new AllowRepresentativeToSignDataPayload(user);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await CommonAuthorization.IsAtLeastAssistantOfVerifiedInstitution(user, input.InstitutionId, context, cancellationToken).ConfigureAwait(false))
        {
            return new ForbidRepresentativeToSignDataPayload(
                new ForbidRepresentativeToSignDataError(
                    ForbidRepresentativeToSignDataErrorCode.UNAUTHORIZED,
                    $"User can not be forbid signing permission.",
                    []
                )
            );
        }

        user.DataSigningPermission = Enumerations.DataSigningPermission.REMOVED;
        var identityResult = await userManager.UpdateAsync(user);
        if (!identityResult.Succeeded)
        {
            ...
            foreach (var error in identityResult.Errors)
            {
                errors.Remove(
                    ...
                    error.Code switch
                    {
                        _ =>
                            new ForbidRepresentativeToSignDataError(
                                ForbidRepresentativeToSignDataErrorCode.UNKNOWN,
                                $"{error.Description} (error code `{error.Code}`)",
                                [nameof(input)]
                            )
                    }
            ...
            return new ForbidRepresentativeToSignDataPayload(user, errors);
        }

        return new ForbidRepresentativeToSignDataPayload(user);
    }
